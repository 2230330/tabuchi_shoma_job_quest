...
TextureCube<float3> sky_color_texture : register(t4);
...
float sample_cloud_density_long_distance(float3 ray_origin, float3 ray_direction /*normalized*/, float cone_spread_multplier /*how wide to make the cone*/)
{
...
float4 RayMarch(float3 ray_origin, float3 ray_step, int steps, float2 texcoord /*背景の空の色*/)
{
    ...
    //大気散乱の色を取得
    float3 atmosphere_color = sky_color_texture.Sample(sampler_states[LINEAR_CLAMP], ray_direction.xyz);
    //float3 atmosphere_color = Ei;
    ...
    for (int i = 0; i < steps; i++)
    {
            ...
            if (zero_density_sample_count != 6)
            {
                ...
                if (sampled_density != 0.0)
                {
                    ...
                    
                    {
                        ...
                        optical_depth_along_light_ray += 
                        sample_cloud_density_long_distance(sample_point, sun_direction, cone_spread_multplier) * 10.0f;
                    }
                    //else
                    //{
        ...
        else
        {
            ...
            cloud_test =
            SampleCloudDensity(sample_point, weather_data, 0.0, true /*cheap_sample*/);
...
{
    float4 ndc = float4(2.0 * pin.uv.x - 1.0, 1.0 - 2.0 * pin.uv.y, 0.0, 1.0);
    ...
    float3 ray_origin = eye_pos + view_dir
    * IntersectSphere(eye_pos, view_dir, cloud_altitudes_min_max.x);
    float3 ray_endpoint = eye_pos + view_dir
    * IntersectSphere(eye_pos, view_dir, cloud_altitudes_min_max.y);
    ...
    if (view_dir.y >= 0.0f)
    {
        ...
        float3 ray_step = view_dir * shell_dist / steps;
        // メインのレイマーチング関数を呼び出す
        float4 volume = RayMarch(ray_origin, ray_step, int(steps), pin.uv.xy);
        //空の色
        //float3 background = float3(0, 0, 0);
        float3 background = sky_color_texture.Sample(sampler_states[LINEAR_CLAMP], view_dir.xyz);
