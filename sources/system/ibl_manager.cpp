...
void IBLManager::Initialize(ID3D11Device* dev)
{
    ...
    // --- Prefilter 用 VS/PS と b0 ---
    {
        ...
        // LatLong->Cubemap用PS
        latlong_to_cube_ps_ = ResourceManager::Instance().LoadPixelShader(dev_.Get(), L".\\resources\\shader\\ibl_sky_atmosphere_ps.cso");
...
{
    //if (!env_source_srv_ || !cs_latlong_to_cube_ || !sky_cube_uav_) return;

    //ID3D11ShaderResourceView* srvs[1] = { env_source_srv_.Get() };
    //ID3D11UnorderedAccessView* uavs[1] = { sky_cube_uav_.Get() };
    //ID3D11SamplerState* samps[1] = { samp_linear_clamp_.Get() };

    //ctx_->CSSetShader(cs_latlong_to_cube_.Get(), nullptr, 0);
    //ctx_->CSSetShaderResources(0, 1, srvs);
    //ctx_->CSSetUnorderedAccessViews(0, 1, uavs, nullptr);
    //ctx_->CSSetSamplers(0, 1, samps);

    //ctx_->Dispatch(kSkyCubeSize / 8, kSkyCubeSize / 8, 1);
    ...
    //// SkyCube が変わったので IBL を dirty に
    //dirty_ = true;

    ////SkyCubeが変わったので保存
    //want_save_dds_ = true;

    if(!env_source_srv_ || !latlong_to_cube_gs_ || !latlong_to_cube_ps_ || !sky_cube_uav_)
        return;
    ...
    //RTV
    ID3D11RenderTargetView* rtv = cube_rtv_all_.Get();
    ...
    //入力SRV
    ID3D11ShaderResourceView* srvs[] = { env_source_srv_.Get() };
    ID3D11SamplerState* sampls[] = { samp_linear_clamp_.Get() };

    ctx_->PSSetShaderResources(0, 1, srvs);
    ctx_->PSSetSamplers(0, 1, sampls);

    //フルスクリーンクワッド
    ctx_->IASetInputLayout(nullptr);
    ctx_->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    ctx_->VSSetShader(vs_fullscreen_.Get(), nullptr, 0);
    ctx_->GSSetShader(latlong_to_cube_gs_.Get(), nullptr, 0);
    ctx_->PSSetShader(latlong_to_cube_ps_.Get(), nullptr, 0);
...
{
    if (!sky_cube_srv_ || !ps_prefilter_ || !vs_fullscreen_ || prefilter_rtvs_.empty())
        return;
    ...
    PrefilterCB cb{}; cb.roughness = roughness; cb.faceIndex = face; cb.mip_count = (float)mipCount;
