	...
	DirectX::XMFLOAT2 viewport_size)
{
	if (is_update_resource)
	{
		...
		if (viewport_size.x < 0)
			view_w = Graphics::Instance().GetScreenWidth();
		else
			view_w = viewport_size.x;
		if (viewport_size.y < 0)
			view_h = Graphics::Instance().GetScreenHeight();
		else
			view_h = viewport_size.y;
		...
		scene.viewport_size.x = view_w;
		scene.viewport_size.y = view_h;
